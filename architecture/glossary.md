...
| **Engagement Read Model (`em_engagement_read_model`)** | A lightweight, queryable projection storing metadata for every engagement (`engagement_id`, `tenant_id`, `template_id`, `current_version_id`, `latest_version_id`, `update_status`). Exists to avoid both rehydration and cross-schema joins on dashboard loads. | Kept in sync by consuming domain events (`EngagementCreated`, `UpdateDecisionRecorded`, `TemplatePublished`). Dashboards always read from here. |
