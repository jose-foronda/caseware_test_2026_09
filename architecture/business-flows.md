...
4. `DiffSummaryService` checks `ds_diff_summary` for an existing row matching `(template_id, from_version_id, to_version_id)`:
   ...
   - **Cache miss:** resolves both blobs using `location_key` from `TemplateQueryService`, computes a JSON diff, generates an LLM narrative, inserts a new `ds_diff_summary` row, returns the narrative.
...
Practitioner clicks "Compare" ──► Dashboard API ──► EMS ──► DiffSummaryService (sync)
                                                               ...
                                                               └── cache miss ──► blob reads + LLM ──► ds_diff_summary ──► returns narrative
